# Tratando uma API
usuario = {
   'nome': 'Joao',
   'redes': [{
      'nome': 'facebook',
      'imagem_profile': '',
      'imagem_capa': 'imagemCapaFacebook' 
   }, {
      'nome': 'twitter',
      'imagem_profile': 'imagemProfileTwitter',
      'imagem_capa': '' 
   }]
}

# Retorna imagem do Profile
def get_imagem_profile(usuario):
   for rede in usuario['redes']:
      if rede['imagem_profile']:
         return rede['imagem_profile']
   return 'default'

# Retorna imagem de imagemCapaFacebook 
def get_imagem_capa(usuario):
   for rede in usuario['redes']:
      if rede['imagem_capa']:
         return rede['imagem_capa']
   return 'default'

print(get_imagem_profile(usuario))
print(get_imagem_capa(usuario))

# Utilizando partial
print('\n### UTILIZANDO PARTIAL ###')
from functools import partial
def get_imagem(qual_imagem,usuario):
   for rede in usuario['redes']:
      if rede[qual_imagem]:
         return rede[qual_imagem]
   return 'default'

get_imagem_profile_partial = partial(get_imagem,'imagem_profile')
get_imagem_capa_partial = partial(get_imagem,'imagem_capa')

print(get_imagem_capa_partial(usuario))
print(get_imagem_profile_partial(usuario))